...
globals [
  ...
  video-filename            ; Output filename for the recorded video
...
; Starts video recording using the vid extension
to start-recording
  set record-video? true
  vid:start-recorder
  output-print "Recording started..."
end

; Stops and saves the video recording to the specified filename
to stop-recording
  vid:save-recording video-filename
  set record-video? false
  output-print (word "Video saved: " video-filename)
end
...
to go
  ...
  if record-video? [ vid:record-view ]   ; Record current frame if video is active
  tick
end
...
"Still Exposed" 1.0 0 -2674135 true "" "plotxy time-min exposed-count"
